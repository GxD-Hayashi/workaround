...
**解決できそうにない場合は、開発チームへ対応要請を行うよう、GMに依頼してください**（解析が途中終了していると判った時点で投げても良いです）。
...
【補足】まとめて確認するコード
```
cd $WORKDIR/$test_type/$batch
for sample in `ls`; do
grep -L "1 of 1 steps (100%) done" $sample/Logs/*.err
done
```
同じ工程を3回行っているものは当該工程の中でエラーが発生し、snakemakeが再トライしたことを示します。\
⇒ 高負荷による実行停止ではないと考えられます。
...
[monitoring inquire](https://github.com/mkaba-gxd/monitoring/blob/v1.3.0/README.md#8-inquire) コマンドで作成することもできます。\
...
※個人情報保護の観点から、要求されたデータの提供については**GMに許可をもらってから**作業してください。
...
WTS Pipeline Fusion解析工程において、解析は正常終了してレポートも作成できているが、domain名が長い、domainの数が多いなどの場合に Fusion Fugure のレイアウトが崩れることがある。
<details>
  <summary> 
    More Details
  </summary>
...
pipelineに不具合があるなど、ログの追跡が必要な場合は、該当する検体のデータは避難させる等の措置を講じてください。
